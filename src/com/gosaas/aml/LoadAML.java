...
/**
 * Servlet implementation class LoadAML
 */
@WebServlet("/LoadAML")
public class LoadAML extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		final String SELECT_AML_SQL = "select * from aml where itemId = ?";
		...
		try {
			try {
				...
				preparedStatement = db.prepareStatement(SELECT_AML_SQL);
		        ...
		        while (rs.next()) {
		        	...
		        	record.put("amlId", rs.getLong("amlId"));
		        	record.put("itemId", rs.getString("itemId"));
	        		record.put("amlName", rs.getString("amlName"));
	        		record.put("amlDescription", rs.getString("amlDescription"));
			  		
			  		array.add(record);
		        }
		        
		        responseObject.put("AML_data", array);
